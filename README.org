...
**Smart contract** Lo smart contract che supporta le operazioni on-chain di FulgurHub è compatibile con la blockchain di Ethereum. In particolare lo smart contract è stato sviluppato in linguaggio Solidity.

**Ambiente di sviluppo maturo** Le motivazioni principali che hanno mosso la scelta di Ethereum riguardano il supporto di smart contract turing completi e l'ambiente di sviluppo maturo. In particolare sono stati utilizzati i seguenti tool e linguaggi: Solidity, ganache, web3. Solidity è il linguaggio di programmazione turing completo con il quale è possibile sviluppare degli smart contract. Ganache è una blockchain di test deployabile in locale, che semplifica la fase di test di uno smart contract. Infine web3 è un'interfaccia in JavaScript che permette di eseguire le operazioni più comuni sulla blockchain di Ethereum. 

**Altre soluzioni** Esistono altre interessanti soluzioni alternative a Ethereum. Una in particolare è Tezos.
